/*
Activity Monitor 1 with head blocker_
source: https://dba.stackexchange.com/questions/165143/how-to-get-the-sql-server-activity-monitors-output-using-t-sql
date: 10/16/2019

* add Where line to show only the blocked and blocking sessions

Problems 
* This query sometimes takes a long time to return. Each of the individual DMVs returns immediately,
but the join of all of them takes a long time. In one case, it took 4+ minutes, in other cases it
ran for 10+ minutes before I killed it. In the former case the one end-user database and everything 
else on the server was healthy. In the latter case, there were about 8,000 connections, but no blocking 
according to Foglight. 
As of 5/27/2025
